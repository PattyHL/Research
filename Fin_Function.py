...
def get_ffme_returns():
    ...
    """
    me_m = pd.read_csv("EDHEC Risk Institute/data/Portfolios_Formed_on_ME_monthly_EW.csv",
...
def get_hfi_returns():
    ...
    """
    hfi = pd.read_csv("EDHEC Risk Institute/data/edhec-hedgefundindices.csv",
...
def get_ind_size():
    """
    """
    ind = pd.read_csv("EDHEC Risk Institute/data/ind30_m_size.csv", header=0, index_col=0)
...
def get_ind_returns():
    ...
    """
    ind = pd.read_csv("EDHEC Risk Institute/data/ind30_m_vw_rets.csv", header=0, index_col=0)/100
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind
...
def get_ind_nfirms():
    """
    """
    ind = pd.read_csv("EDHEC Risk Institute/data/ind30_m_nfirms.csv", header=0, index_col=0)/100
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind
...
def run_cppi(risky_r, safe_r=None, m=3, start=1000, floor=0.8, riskfree_rate=0.03, drawdown=None):
    ...
    import pandas as pd
    """
    Run a backtest of the CPPI strategy, given a set of returns for the risk asset
    ...
    risk_w_history = pd.DataFrame().reindex_like(risky_r)
    ...
    for step in range(n_steps):
        ...
        risk_w_history.iloc[step] = risk_w
        account_history.iloc[step] = account_value
    risk_wealth = start*(1+risky_r).cumprod()
    backtest_result = {
        "Wealth": account_history,
        "Risk Wealth": risk_wealth, 
        "Risk Budget": cushion_history,
        "Risk Allocation": risk_w_history,
        "m": m,
        "Start": start,
        "Floor": floor,
        "risky_r":risky_r,
...
def ann_to_inst(r):
    """
    Convert an annualized interest rate to an instantaneous interest rate
    """
    return np.log1p(r)
...
def bond_price(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
    ...
    """
    if isinstance(discount_rate, pd.DataFrame):  #isinstance --> check if number is integer
